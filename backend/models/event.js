...
module.exports = {
  ...
  /**
   * Approve team registration for an event
   * @param {string} eventId - Event ID
   * @param {string} teamId - Team ID
   * @returns {Promise<boolean>} - Promise resolving to true if successful
   */
  approveTeamRegistration: async function(eventId, teamId) {
    try {
      const result = await Event.updateOne(
        { 
          _id: eventId, 
          'team_registrations.team_id': teamId 
        },
        { 
          $set: { 
            'team_registrations.$.status': 'approved',
            'team_registrations.$.approval_date': new Date()
          } 
        }
      );
  ...
  /**
   * Reject team registration for an event
   * @param {string} eventId - Event ID
   * @param {string} teamId - Team ID
   * @returns {Promise<boolean>} - Promise resolving to true if successful
   */
  rejectTeamRegistration: async function(eventId, teamId) {
    try {
      const result = await Event.updateOne(
        { 
          _id: eventId, 
        ...
        { 
          $set: { 
            'team_registrations.$.status': 'rejected',
            'team_registrations.$.rejection_date': new Date()
          } 
        }
      );
