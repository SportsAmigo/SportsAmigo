        ...
        <AdminLayout>
            <div className="p-6">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-600">Manage and monitor all player accounts</p>
                </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1">
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                                    value={searchTerm}
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className={`px-6 py-3 rounded-lg font-semibold transition-all ${filterStatus === 'all' ? 'bg-slate-700 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                            >
                                All
                            </button>
                            <button
                                onClick={() => setFilterStatus('active')}
                                className={`px-6 py-3 rounded-lg font-semibold transition-all ${filterStatus === 'active' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                            >
                                Active
                            </button>
                            <button
                                onClick={() => setFilterStatus('inactive')}
                                className={`px-6 py-3 rounded-lg font-semibold transition-all ${filterStatus === 'inactive' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                            >
                                Inactive
                            </button>
                        </div>
                ...
                <div className="bg-white rounded-xl shadow-md overflow-hidden">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="w-10 h-10 bg-violet-100 rounded-full flex items-center justify-center">
                                                        <i className="fas fa-user text-violet-600"></i>
                                                    </div>
                                                    <span className="ml-3 font-medium text-gray-900">{player.name}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-gray-600">{player.email}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                                                    {player.sport || 'Unspecified'}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-gray-600">{player.team || 'Unassigned'}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-gray-600 text-sm">
                                                {player.joinedDate || 'N/A'}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-3 py-1 rounded-full text-xs font-medium ${player.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
                    ...
                    {/* Pagination */}
                    {!loading && filteredPlayers.length > 0 && totalPages > 1 && (
